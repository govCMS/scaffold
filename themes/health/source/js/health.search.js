...
(function ($, Drupal) {
  ...
  Drupal.behaviors.healthSearch = {
    attach: function (context, settings) {
      ...
      $('.health-sub-nav ul.au-link-list li').once().clone().insertAfter('.au-main-nav ul li:last-of-type').addClass('au-main-nav--mobile-only');

      // Search button handler.
      $('.au-main-nav__toggle--search').once().click(function (e) {
        $('.health-search--global').toggle();
        $(this).find('span.svg-inline--fa').toggle();
      });

      // Filter toggles.
      $('.health-filter').once().click(function (e) {
      ...
      $('.region-navigation #search-api-page-search-form-default-search--2').once().addClass('col-xs-12');
